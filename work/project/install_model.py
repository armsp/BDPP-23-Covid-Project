...
#copied from https://stackoverflow.com/a/48450305/6282134
...
def create_parent_directories(file_path):
    ...
    if not os.path.exists(directory):
        ...
        print("Parent directories already exist.")

def install_model( path, model_name ):

    # replace with actual args from command line
    #path = "full.pkl"
    #model_name = "train_honest_forward"
    ...
    node = nodes.find( model_name ).given( )

    cache_key = node.get_cache_key( )
    cache_key += ".pkl"
    #cache_key = '1717177/d52ab39.pkl'

    file_path = f"./node_cache/{ cache_key }" 

    if os.path.exists( file_path ):

        print( f"model '{ model_name }' is already installed at '{ file_path }'" )
        test_load( )
        exit( )
    
    create_parent_directories( file_path )
    print( f"installing model '{ model_name }' from '{ path }' to '{ file_path }'..." )
    print( "if this takes too long, (it is just a copy), you can execute this manually, this is much faster in mounted docker directories" )
    copy_with_progress( path, file_path ) 
    print( "done" )
    test_load( )
